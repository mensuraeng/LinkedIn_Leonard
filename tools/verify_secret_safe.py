...
ASSIGNMENT = re.compile(
    r"(?i)(?<![a-z0-9_])['\"]?(?:[a-z0-9]+_)*(?:api[_-]?key|client[_-]?secret|access[_-]?token|refresh[_-]?token|password)['\"]?(?:\s*:\s*[a-z_][a-z0-9_]*(?:\[[^\]]+\])?)?\s*[:=]\s*(?!(?:['\"])?(?:\$\{[^}]+\}|secret://[^\s'\",;)\]}#]+)(?:['\"])?(?=\s|$|[, ;)\]}.#]))(?:\(\s*)?(?:(?:[bBfFrR]{0,2})?['\"][^'\"]{8,}['\"]|(?:[bBfFrR]{0,2})?['\"][^'\"]+['\"]\s+(?:[bBfFrR]{0,2})?['\"][^'\"]+['\"]|[^\s#'\"]{8,})"
)
