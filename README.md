...
- `MockLinkedInGateway` existe apenas em memória, aceita falhas injetadas (`401`, `403`, `429`, `timeout`) e deduplica somente quando a chave de idempotência e o fingerprint ação/conta/payload são idênticos; colisões são recusadas.
- `Simulator` confirma uma missão somente depois de resultado positivo do gateway e da verificação.
